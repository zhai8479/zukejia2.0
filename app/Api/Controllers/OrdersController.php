...
//        $orders['service_time'] = date("Y-m-d H:i:s");
    ...
    public function show($id)
    {
        ...
        if ($order) {
            ...
            $apartment =  Apartment::where('id',$order->apartment_id)->get();
            $result = [];
            $apartment->reject(function($item)use(&$result, $apartment){
                $result[] = $item->indexListFilter($item);
            });

            $order->apartment_info =$result[0];
        ...
        if ($pay_type == 1) {
            ...
            try{
              /*  //订单支付渠道改为支付宝
                ...
                //支付宝回调
                $request->setNotifyUrl("http://a1.zukehouse.com/api/order/alipayNotify");
...
//            @file_put_contents("pay_order_no.php",$pay_order_no);
           ...
           // AlipayLog::create(['log_text11'=>'直接调用结束']);
            ...
            AlipayLog::create(['log_text'=>$ex->getMessage()]);
            return 'fail';
        }
    }
    /**
